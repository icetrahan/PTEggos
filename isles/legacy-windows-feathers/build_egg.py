...
egg = {
    ...
    "variables": [_v(*a) for a in [
        ...
        ("Multihome Auto", "MULTIHOME_AUTO", "0", "required|string|in:0,1", "OPT-IN (#1832 / #97). 1 = when MULTIHOME_IP is blank, fall back to this server's OWN allocation IP (SERVER_IP), the way start-evrima.ps1 already does. PROVEN 2026-09-05 (multihome-97-0905): on feathers SERVER_IP is the default-allocation IP, and with AUTO=1 a SECOND Legacy server on the same box kept its own 7778 (Steam init OK, A2S on its IP, listed on Steam at its IP) where AUTO=0 made it fail Steam init. Default stays 0 = bind all interfaces, unchanged for every existing server (Ice's 2026-07-27 ruling) - flipping the default is Ice's call. On 0.0.0.0 the Steam query port is a BOX-WIDE namespace and the first Legacy server on the box owns it on every IP."),
    ]],
}
