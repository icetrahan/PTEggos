...
egg = {
    ...
    "variables": [_v(*a) for a in [
        ...
        ("Multihome Mode", "MULTIHOME_MODE", "cli", "required|string|in:cli", "How a multihome IP is applied. MEASURED 2026-09-05 (#97 A/B/A on win3, two servers): cli = -MULTIHOME=<ip> engine arg is the ONLY form that binds the IP. url (?MultiHome=<ip> in the travel URL) and both bind 0.0.0.0 - the URL option negates the arg. Pinned to cli; the wrapper honours a stale url/both value as cli and says so."),
        ("Multihome Auto", "MULTIHOME_AUTO", "1", "required|string|in:0,1", "DEFAULT 1 since 2026-09-06 (Ice, after G12: two multihomed Legacy servers proven on win3 incl. a human join to each by IP). 1 = when MULTIHOME_IP is blank, bind/advertise this server's OWN allocation IP (SERVER_IP, real on feathers = the default-allocation IP) via -MULTIHOME=<ip>; 0 = bind ALL interfaces (0.0.0.0), where the Steam query port is a BOX-WIDE namespace and the first Legacy server on the box owns it on every IP (#1832). Reaches NEW provisions only - an existing volume keeps the wrapper it installed with, and a per-server value set in the panel always wins over this default. #2038: a Legacy server binds THREE udp ports (game, QueryPort, QueryPort+1 = Steam master) so the billing ServerTemplate must hand out 3 allocations, like Evrima.")
    ]],
}
